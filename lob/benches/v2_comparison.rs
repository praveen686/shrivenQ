...
use lob::{OrderBook, OrderBookV2, CrossResolution};
...
fn bench_cross_resolution(c: &mut Criterion) {
    ...
    for i in (0..updates.len()).step_by(10) {
        if i + 1 < updates.len() {
            ...
            updates[i + 1] = L2Update::new(
                Ts::from_nanos((i + 1) as u64),
                Symbol(1),
                Side::Ask,
                Px::new(99.0),
                Qty::new(100.0),
...
fn bench_imbalance_calculation(c: &mut Criterion) {
    ...
    c.bench_function("v1_imbalance", |b| {
        b.iter(|| {
            ...
            black_box(if total > 0.0 { (bid_vol - ask_vol) / total } else { 0.0 });
        });
    });
