...
impl FeatureCalculatorV2 {
    ...
    pub fn calculate(&mut self, book: &OrderBookV2) -> Option<FeatureFrameV2> {
        ...
        self.spread_ema = self.spread_ema * 0.95 + self.last_spread * 0.05;
        self.update_count += 1;
    ...
    fn calculate_weighted_spread(&self, book: &OrderBookV2) -> f64 {
        ...
        for i in 0..5 {
            if let (Some(bid), Some(ask)) = (
                self.get_level(&book.bids, i),
                self.get_level(&book.asks, i)
            ) {
    ...
    fn calculate_flow_toxicity(&self, book: &OrderBookV2) -> f64 {
        ...
        let volume_ratio = book.asks.total_volume() as f64 / 
                          (book.bids.total_volume() as f64 + 1.0);
    ...
    fn forecast_volatility(&self) -> f64 {
        ...
        let variance = self.volatility_window.iter()
            .map(|v| (v - mean).powi(2))
            .sum::<f64>() / self.volatility_window.len() as f64;
    ...
    fn estimate_current_volatility(&self) -> f64 {
        if self.volatility_window.len() >= 10 {
            let recent: Vec<_> = self.volatility_window.iter()
                .rev()
                .take(10)
                .copied()
                .collect();
