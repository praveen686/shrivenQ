...
impl OrderBookV2 {
    ...
    pub fn new_with_roi(
        ...
        let roi_lb = roi_center - roi_width / 2.0;
        let roi_ub = roi_center + roi_width / 2.0;
        
        book.bids.init_roi(roi_lb, roi_ub, tick_size);
        book.asks.init_roi(roi_lb, roi_ub, tick_size);
    ...
    pub fn apply_validated(&mut self, update: &L2Update) -> Result<(), LobV2Error> {
        ...
        if update.side == Side::Bid {
            self.bids.update(update.price, update.qty, update.ts, update.level)?;
        } else if update.side == Side::Ask {
            self.asks.update(update.price, update.qty, update.ts, update.level)?;
        } else {
            return Ok(());
        }
    ...
    pub fn apply_fast(&mut self, update: &L2Update) {
        if update.side == Side::Bid {
            let _ = self.bids.update_fast(update.price, update.qty, update.level);
        } else if update.side == Side::Ask {
            let _ = self.asks.update_fast(update.price, update.qty, update.level);
        }
    ...
    pub fn microprice(&self) -> Option<f64> {
        match (self.best_bid(), self.best_ask()) {
            (Some((bid_px, bid_qty)), Some((ask_px, ask_qty))) => {
                ...
                if total_weight > 0.0 {
                    Some(
                        (bid_px.as_f64() * ask_weight + ask_px.as_f64() * bid_weight)
                            / total_weight,
                    )
                } else {
                    self.mid_price()
    ...
    pub fn spread_ticks(&self) -> Option<i64> {
        match (self.best_bid(), self.best_ask()) {
            (Some((bid, _)), Some((ask, _))) => {
                Some(((ask.as_f64() - bid.as_f64()) / self.tick_size) as i64)
            }
            _ => None,
        }
    }
...
impl SideBookV2 {
    ...
    pub fn init_roi(&mut self, roi_lb: f64, roi_ub: f64, tick_size: f64) {
        self.roi_lb_tick = (roi_lb / tick_size).round() as i64;
        self.roi_ub_tick = (roi_ub / tick_size).round() as i64;
        let roi_size = (self.roi_ub_tick - self.roi_lb_tick + 1) as usize;
    ...
    pub fn update(
        ...
        if level < 32 {
            self.update_array(level as usize, price, qty, ts)?;
            return Ok(());
        }
        ...
        if price_tick >= self.roi_lb_tick && price_tick <= self.roi_ub_tick {
            ...
            if qty.is_zero() && price_tick == self.best_price_tick {
                ...
                if self.is_bid {
                    ...
                    if price_tick < self.best_price_tick || self.best_price_tick == INVALID_MAX {
                        self.best_price_tick = price_tick;
                    }
                }
            }
    ...
    fn update_array(&mut self, level: usize, price: Px, qty: Qty, ts: Ts) -> Result<(), LobV2Error> {
        if level >= 32 {
        ...
        if self.is_bid {
            ...
            for i in (0..self.roi_qtys.len()).rev() {
                if !self.roi_qtys[i].is_zero() {
                    self.best_price_tick = self.roi_lb_tick + i as i64;
                    return;
                }
            }
    ...
    #[inline]
    pub fn get_level(&self, level: usize) -> Option<(Px, Qty)> {
    ...
    pub fn clear_below_or_equal(&mut self, price: Px) {
        ...
        if !self.roi_qtys.is_empty() {
            ...
            if price_tick >= self.roi_lb_tick {
                let clear_up_to = ((price_tick - self.roi_lb_tick + 1) as usize).min(self.roi_qtys.len());
    ...
    pub fn clear_above_or_equal(&mut self, price: Px) {
        ...
        if !self.roi_qtys.is_empty() {
            ...
            if price_tick <= self.roi_ub_tick {
                let clear_from = ((price_tick - self.roi_lb_tick) as usize).max(0);
...
mod tests {
    ...
    fn test_cross_resolution() {
        ...
        // Set up initial book
        book.apply_validated(&L2Update::new(
            Ts::from_nanos(1),
            Symbol(1),
            Side::Bid,
            Px::new(100.0),
