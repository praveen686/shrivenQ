...
fn main() {
    ...
    for regime_phase in 0..4 {
        ...
        for i in 0..100 {
            ...
            // Create ask update
            let ask_update = L2Update::new(
                Ts::from_nanos(ts + 1000),
                symbol,
                Side::Ask,
                Px::new(mid_price + spread / 2.0),
                Qty::new(100.0 + rng.r#gen::<f64>() * 900.0),
            ...
            if rng.r#gen::<f64>() < 0.05 {
                ...
                let shock = L2Update::new(
                    Ts::from_nanos(ts + 2000),
                    symbol,
                    side,
    ...
    if let Some(final_features) = hft_calc.calculate(&book) {
        ...
        println!("  Mean Reversion Signal: {:.4}", final_features.mean_reversion_signal);
        println!("  Momentum: {:.4}", final_features.momentum);
        println!("  Adverse Selection Risk: {:.2}%", final_features.adverse_selection * 100.0);
    ...
    if let Some(mm_features) = mm_calc.calculate(&book) {
        ...
        println!("  Effective Spread: {:.2} bps", mm_features.effective_spread);
