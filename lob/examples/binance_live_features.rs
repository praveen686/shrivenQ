...
use lob::{OrderBookV2, features_v2, CrossResolution, MarketRegime};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
                            ...
                            if let Some(features) = hft_calc.calculate(&book) {
                                ...
                                if update_count % 100 == 0 {
                                    if let (Some((bid_px, bid_qty)), Some((ask_px, ask_qty))) = 
                                        ...
                                        info!("  BBO: {:.2} x {:.3} | {:.2} x {:.3}",
                                            bid_px.as_f64(), bid_qty.as_f64(),
                                            ask_px.as_f64(), ask_qty.as_f64());
                                        info!("  Spread: {} ticks ({:.2} bps)",
                                ...
                                if update_count % 200 == 0 {
                                    ...
                                    if let Some(mm_features) = mm_calc.calculate(&book) {
                                        info!("  MM Metrics:");
                                        info!("    Effective Spread: {:.2} bps", mm_features.effective_spread);
                                        info!("    Price Impact: {:.2} bps", mm_features.price_impact);
    ...
    if !feature_frames.is_empty() {
        let avg_spread: f64 = feature_frames.iter()
            .map(|f| f.weighted_spread)
            .sum::<f64>() / feature_frames.len() as f64;
        
        let avg_toxicity: f64 = feature_frames.iter()
            .map(|f| f.flow_toxicity)
            .sum::<f64>() / feature_frames.len() as f64;
        
        let avg_liquidity: f64 = feature_frames.iter()
            .map(|f| f.liquidity_score)
            .sum::<f64>() / feature_frames.len() as f64;
        
        let volatility_max = feature_frames.iter()
        ...
        if avg_toxicity < 0.3 && avg_liquidity > 0.6 {
            ...
            info!("  Use adaptive spread: {:.2}-{:.2} bps", 
                avg_spread * 0.9, avg_spread * 1.2);
        }
    }
