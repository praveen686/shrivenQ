...
use lob::{OrderBookV2, CrossResolution, features_v2, MarketRegime};
    ...
    pub fn to_timestamp_ns(&self) -> Result<u64> {
        ...
        if self.timestamp.contains(':') {
            ...
            let time_parts: Vec<&str> = if self.timestamp.contains(' ') {
                self.timestamp.split(' ').nth(1).unwrap_or("").split(':').collect()
            } else {
        ...
        if let Ok(epoch_ms) = self.timestamp.parse::<u64>() {
            if epoch_ms > 1000000000000 { // Looks like milliseconds
                return Ok(epoch_ms * 1_000_000);
            } else if epoch_ms > 1000000000 { // Looks like seconds
...
impl LobEstimator {
        ...
        if let (Some(last_mid), Some(current_bid), Some(current_ask)) = 
            ...
            if tick.price > mid + self.tick_size / 2.0 {
                ...
                if tick.price > last_mid {
                    ...
                    let net_direction: i32 = recent_trades.map(|t| {
                        if t.price > last_mid { 1 } else if t.price < last_mid { -1 } else { 0 }
                    }).sum();
                    
                    return if net_direction >= 0 { Side::Bid } else { Side::Ask };
                }
            }
        }
        
        // Default: alternate based on trade count
        if self.total_trades % 2 == 0 { Side::Bid } else { Side::Ask }
    }
    ...
    fn update_depth_estimates(&mut self) {
        ...
        for (price_ticks, total_volume) in price_volume_map {
            ...
            if estimated_quantity > 1.0 { // Minimum quantity threshold
...
impl NiftyTickToLob {
    ...
    pub fn process_tick_file(&mut self, path: &Path, symbol_filter: Option<&str>) -> Result<Vec<(OrderBookV2, Ts)>> {
            ...
            match NiftyTick::from_csv_line(&line) {
                Ok(tick) => {
                    ...
                    if processed_ticks % 1000 == 0 {
                        info!("Processed {} ticks - {}", processed_ticks, self.estimator.get_stats());
                    }
                }
                Err(e) => {
                    if line_count <= 10 { // Only warn for first few errors
        ...
        info!("Processed {} ticks from {} lines", processed_ticks, line_count);
        ...
        if !lob_snapshots.is_empty() {
            ...
            for (i, (book, ts)) in lob_snapshots.iter().enumerate().take(10) {
                if let Some(features) = feature_calc.calculate(book) {
                    info!("Snapshot {}: BBO={:.2}x{:.0}|{:.2}x{:.0}, Spread={:.2}, Regime={:?}", 
                        i + 1,
                        book.best_bid().map(|(p, q)| (p.as_f64(), q.as_f64())).unwrap_or((0.0, 0.0)).0,
                        book.best_bid().map(|(p, q)| (p.as_f64(), q.as_f64())).unwrap_or((0.0, 0.0)).1,
                        book.best_ask().map(|(p, q)| (p.as_f64(), q.as_f64())).unwrap_or((0.0, 0.0)).0,
                        book.best_ask().map(|(p, q)| (p.as_f64(), q.as_f64())).unwrap_or((0.0, 0.0)).1,
