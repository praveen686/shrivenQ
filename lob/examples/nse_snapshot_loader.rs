...
use lob::{OrderBookV2, CrossResolution};
...
impl NseOrderEntry {
    ...
    pub fn from_csv_line(line: &str) -> Result<Self> {
        ...
        if fields.len() < 18 {
            anyhow::bail!("Invalid line format: expected 18 fields, got {}", fields.len());
        }
        ...
        Ok(Self {
            ...
            side: if fields[10] == "B" { Side::Bid } else { Side::Ask },
...
impl NseSnapshotLoader {
    ...
    pub fn build_lob(&mut self, entries: &[NseOrderEntry], symbol_filter: Option<&str>) -> Result<OrderBookV2> {
        ...
        info!("Building LOB for {} with {} orders", symbol_str, filtered.len());
        ...
        for entry in &filtered {
            ...
            levels.get_mut(&price_ticks).unwrap().orders.push(entry.clone());
        }
        ...
        info!("LOB built with {} bid levels and {} ask levels", 
            bid_levels.len().min(20), ask_levels.len().min(20));
    ...
    pub fn analyze_snapshot(&self, entries: &[NseOrderEntry], symbol: Option<&str>) {
        ...
        let total_buy_qty: f64 = filtered.iter()
            .filter(|e| e.side == Side::Bid)
            .map(|e| e.quantity)
            .sum();
        
        let total_sell_qty: f64 = filtered.iter()
        ...
        let best_bid = filtered.iter()
            .filter(|e| e.side == Side::Bid)
            .map(|e| e.price)
            .max_by(|a, b| a.partial_cmp(b).unwrap());
        
        let best_ask = filtered.iter()
            .filter(|e| e.side == Side::Ask)
            .map(|e| e.price)
        ...
        println!("Buy Orders: {} ({:.1}%)", buy_orders, buy_orders as f64 / total_orders as f64 * 100.0);
        println!("Sell Orders: {} ({:.1}%)", sell_orders, sell_orders as f64 / total_orders as f64 * 100.0);
        println!("Total Buy Quantity: {:.0}", total_buy_qty);
        println!("Total Sell Quantity: {:.0}", total_sell_qty);
        println!("Order Imbalance: {:.2}", (total_buy_qty - total_sell_qty) / (total_buy_qty + total_sell_qty));
        println!("Average Order Size: {:.0}", avg_order_size);
        
        if let (Some(bid), Some(ask)) = (best_bid, best_ask) {
            println!("Best Bid: {:.2}", bid);
            println!("Best Ask: {:.2}", ask);
            println!("Spread: {:.2} ({:.2} bps)", ask - bid, (ask - bid) / bid * 10000.0);
        }
        ...
        println!("\n📋 Order Types:");
        println!("Disclosed Quantity Orders: {} ({:.1}%)", 
            disclosed_orders, disclosed_orders as f64 / total_orders as f64 * 100.0);
        println!("GTD Orders: {} ({:.1}%)", 
            gtd_orders, gtd_orders as f64 / total_orders as f64 * 100.0);
        ...
        for (book_type, count) in book_types {
            println!("  {}: {} ({:.1}%)", 
                book_type, count, count as f64 / total_orders as f64 * 100.0);
        }
    }
}
    ...
    if snapshot_path.exists() {
        ...
        if let Ok(book) = loader.build_lob(&entries, Some("NIFTY")) {
            if let (Some((bid_px, bid_qty)), Some((ask_px, ask_qty))) = 
                ...
                info!("Spread: {:.2} bps", book.spread_ticks().unwrap_or(0) as f64 * 0.01);
            }
        }
        ...
        for symbol in ["ACC", "RELIANCE", "TCS", "INFY"] {
            if let Ok(book) = loader.build_lob(&entries, Some(symbol)) {
                info!("\n📊 {} Order Book built successfully", symbol);
            }
        }
    } else {
