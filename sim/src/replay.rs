...
pub struct Replayer<P: Publisher<ReplayEvent>> {
    wal_path: PathBuf,
    publisher: P,
...
impl<P: Publisher<ReplayEvent> + Clone + Send + 'static> Replayer<P> {
    ...
    async fn replay_loop(
            ...
            while let Some(event) = iter.next()? {
                ...
                if cfg.speed > 0.0 {
                    if let Some(last) = last_ts {
                        ...
                        if replay_delay_ns > 0.0 {
                            let delay = Duration::from_nanos(replay_delay_ns as u64);
                            sleep(delay).await;
                        }
                    }
                }
        ...
        info!(
            ...
            prog.events_processed,
            prog.start_time.map(|s| s.elapsed()).unwrap_or_default()
        );
