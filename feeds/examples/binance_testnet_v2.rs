...
use feeds::{FeedAdapter, FeedConfig, BinanceWebSocketFeed};
use auth::{BinanceAuth, BinanceConfig, BinanceMarket};
use common::{Symbol, L2Update};
...
impl Metrics {
    ...
    fn report(&self) {
        ...
        info!("  Updates Processed: {}", self.updates_processed.load(Ordering::Relaxed));
        info!("  Crossed Books Detected: {}", self.crossed_books_detected.load(Ordering::Relaxed));
        info!("  Crossed Books Resolved: {}", self.crossed_books_resolved.load(Ordering::Relaxed));
        info!("  Validation Errors: {}", self.validation_errors.load(Ordering::Relaxed));
        info!("  Latencies:");
        info!("    Average: {} ns", self.avg_latency_ns.load(Ordering::Relaxed));
        info!("    P50: {} ns", self.p50_latency_ns.load(Ordering::Relaxed));
        info!("    P95: {} ns", self.p95_latency_ns.load(Ordering::Relaxed));
        info!("    P99: {} ns", self.p99_latency_ns.load(Ordering::Relaxed));
    }
}
        ...
        match tokio::time::timeout(Duration::from_secs(1), rx.recv()).await {
            Ok(Some(update)) => {
                ...
                let result = match update.symbol {
                    s if s == btcusdt => {
                        btc_book.apply_validated(&update)
                    }
                    s if s == ethusdt => {
                        eth_book.apply_validated(&update)
                    }
                    _ => Ok(()),
                };
                ...
                if last_report.elapsed() > Duration::from_secs(5) {
                    metrics.updates_processed.store(update_count, Ordering::Relaxed);
                    metrics.crossed_books_detected.store(crossed_detections, Ordering::Relaxed);
                    metrics.crossed_books_resolved.store(crossed_resolutions, Ordering::Relaxed);
...
fn log_book_state(name: &str, book: &OrderBookV2) {
    ...
    if let Some((bid_px, bid_qty)) = book.best_bid() {
        info!("  Best Bid: {:.2} @ {:.4}", bid_px.as_f64(), bid_qty.as_f64());
    } else {
        info!("  Best Bid: None");
    }
    
    if let Some((ask_px, ask_qty)) = book.best_ask() {
        info!("  Best Ask: {:.2} @ {:.4}", ask_px.as_f64(), ask_qty.as_f64());
    } else {
        info!("  Best Ask: None");
    }
