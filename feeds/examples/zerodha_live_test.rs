...
use feeds::zerodha_ws::ZerodhaWebSocketFeed;
use feeds::adapter::{FeedConfig, FeedAdapter};
use common::Symbol;
use lob::OrderBookV2;
use tokio::sync::mpsc;
...
async fn main() -> anyhow::Result<()> {
    ...
    let is_market_hours = (hour == 9 && minute >= 15) || (hour > 9 && hour < 15) || (hour == 15 && minute <= 30);
    ...
    let mut ws_feed = ZerodhaWebSocketFeed::new(
        feed_config.clone(),
        auth,
    );
    ...
    ws_feed.subscribe(vec![Symbol(256265), Symbol(260105)]).await?;
    ...
    while let Some(update) = rx.recv().await {
        ...
        if update_count % 50 == 0 {
            ...
            if let (Some((bid, bid_qty)), Some((ask, ask_qty))) = 
                ...
                info!("  NIFTY: {:.2} x {} | {:.2} x {} (spread: {:.2})",
                    bid.as_f64(), bid_qty.as_i64(),
                    ask.as_f64(), ask_qty.as_i64(),
            ...
            if let (Some((bid, bid_qty)), Some((ask, ask_qty))) = 
                ...
                info!("  BANKNIFTY: {:.2} x {} | {:.2} x {} (spread: {:.2})",
                    bid.as_f64(), bid_qty.as_i64(),
                    ask.as_f64(), ask_qty.as_i64(),
                    ask.as_f64() - bid.as_f64()
                );
    ...
    info!("Average rate: {:.1} updates/sec", 
        update_count as f64 / start_time.elapsed().as_secs_f64());
