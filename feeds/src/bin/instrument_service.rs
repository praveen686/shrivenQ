...
async fn main() -> Result<()> {
    ...
    match cli.command.unwrap_or(Commands::Run { once: false }) {
        ...
        Commands::Show { symbol, indices, futures } => {
            // Load from cache
            ...
            if indices {
                ...
                info!("\n📈 Active futures for {} ({}):", underlying, futures.len());
                for future in futures {
                    info!("  {} - {} (expires: {})", 
                        future.instrument_token,
                        future.trading_symbol,
                        future.expiry.map(|e| e.format("%Y-%m-%d").to_string())
                ...
                if instruments.is_empty() {
                    ...
                    info!("\n🔍 Instruments matching '{}' ({}):", symbol, instruments.len());
        ...
        Commands::Validate => {
            ...
            let store = fetcher.store();
            store.load_from_cache(cache_file.to_str().unwrap()).await?;
            info!("✅ Instruments file valid: {} instruments", store.count().await);
