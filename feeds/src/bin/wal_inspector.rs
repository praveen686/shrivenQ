...
fn extract_tokens(buffer: &[u8]) -> HashMap<u32, usize> {
    ...
    for (start, end) in token_ranges {
        for token in start..=end {
            ...
            let count = buffer.windows(4)
                .filter(|w| *w == token_bytes)
                .count();
            if count > 0 {
...
fn show_summary(file_path: &str, with_lob: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("By Category:");
    println!("  Spot:    {:6} ticks ({:.1}%)", spot_count, 
        if total_count > 0 { (spot_count as f64 / total_count as f64) * 100.0 } else { 0.0 });
    println!("  Futures: {:6} ticks ({:.1}%)", future_count,
        if total_count > 0 { (future_count as f64 / total_count as f64) * 100.0 } else { 0.0 });
    println!("  Options: {:6} ticks ({:.1}%)", option_count,
        if total_count > 0 { (option_count as f64 / total_count as f64) * 100.0 } else { 0.0 });
    ...
    if with_lob {
        ...
        if std::path::Path::new(lob_path).exists() {
            ...
            let snapshot_count = lob_buffer.windows(8)
                .filter(|w| w == b"snapshot")
                .count();
