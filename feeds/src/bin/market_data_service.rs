...
use feeds::{
    InstrumentFetcher, InstrumentFetcherConfig,
    MarketDataPipeline, PipelineConfig,
    ZerodhaWebSocketFeed,
    FeedAdapter, FeedConfig,
};
...
async fn main() -> Result<()> {
    ...
    match command {
        Commands::Run { symbols, strike_range, dry_run } => {
...
async fn run_service(
    ...
    // Parse symbols
    let spot_symbols: Vec<String> = symbols
        .split(',')
    ...
    if cache_file.exists() {
        ...
        instrument_store.load_from_cache(cache_file.to_str().unwrap()).await?;
    ...
    let pipeline = Arc::new(
        MarketDataPipeline::new(pipeline_config, instrument_store.clone()).await?
    );
...
async fn fetch_instruments(cli: &Cli) -> Result<()> {
    ...
    let mut fetcher = InstrumentFetcher::new(
        fetcher_config,
        Some(zerodha_auth),
        None,
    )?;
