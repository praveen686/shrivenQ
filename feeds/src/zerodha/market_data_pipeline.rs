...
use lob::{OrderBookV2, FeatureCalculatorV2};
use storage::{Wal, WalEvent, TickEvent};
...
impl MarketDataPipeline {
    ...
    pub async fn new(
        ...
        // Initialize WALs
        let tick_wal = Arc::new(RwLock::new(
            Wal::new(&tick_dir, Some(config.wal_segment_size))?
        ));
        
        let lob_wal = Arc::new(RwLock::new(
            Wal::new(&lob_dir, Some(config.wal_segment_size))?
        ));
    ...
    pub async fn initialize_subscriptions(&self) -> Result<()> {
        ...
        for spot_symbol in &self.config.spot_symbols {
            ...
            info!("Setting up subscription for {} (token: {})", 
            ...
            for token in &subscription.tokens {
                ...
                feature_calcs.insert(
                    symbol,
                    FeatureCalculatorV2::new(symbol)
                );
            }
    ...
    async fn build_subscription(&self, spot: Instrument) -> Result<InstrumentSubscription> {
        ...
        let futures = self.instrument_store
        ...
        if let Some(fut) = sorted_futures.get(1) {
            ...
            subscription.tokens.insert(fut.instrument_token);
            info!("  Next future: {} (expires: {:?})", 
                fut.trading_symbol, fut.expiry);
        }
        
        // Get current expiry for options
        if let Some(current_expiry) = subscription.current_future
            .as_ref()
            .and_then(|f| f.expiry) {
            
            // Get option chain with correct symbol
            let options = self.instrument_store
            ...
            let atm_strike = (spot_price / self.config.strike_interval).round() 
                * self.config.strike_interval;
            
            // Select strikes within range
            let min_strike = atm_strike - (self.config.option_strike_range as f64 
                * self.config.strike_interval);
            let max_strike = atm_strike + (self.config.option_strike_range as f64 
                * self.config.strike_interval);
            ...
            info!("  Options: {} calls, {} puts (strikes {:.0}-{:.0})",
            ...
            if let WalEvent::Tick(tick) = event {
                if tick.symbol == symbol {
                    ...
                    if let (Some(bid), Some(ask)) = (tick.bid, tick.ask) {
                        ...
                        let updates = vec![
                            L2Update::new(tick.ts, symbol, Side::Bid, bid, Qty::new(1.0), 0),
                            L2Update::new(tick.ts, symbol, Side::Ask, ask, Qty::new(1.0), 0),
                        ];
    ...
    pub async fn start(self: Arc<Self>) -> Result<()> {
        ...
        tokio::spawn(async move {
            let mut interval = interval(Duration::from_secs(
                pipeline.config.snapshot_interval_secs
            ));
