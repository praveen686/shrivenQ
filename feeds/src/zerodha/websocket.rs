...
impl ZerodhaWebSocketFeed {
    ...
    async fn run_websocket(&mut self, tx: mpsc::Sender<L2Update>) -> anyhow::Result<()> {
        ...
        debug!("API Key length: {}, Access token length: {}", 
            api_key.len(), access_token.len());
        ...
        let tokens: Vec<u32> = self.symbols
            .iter()
                        ...
                        for i in 0..5 {
                            ...
                            if level_offset + 12 <= packet.len() {
                                let qty = u32::from_be_bytes([
                                    packet[level_offset], packet[level_offset + 1],
                                    packet[level_offset + 2], packet[level_offset + 3]
                                ]);
                                let price = u32::from_be_bytes([
                                    packet[level_offset + 4], packet[level_offset + 5],
                                    packet[level_offset + 6], packet[level_offset + 7]
                                ]);
                                ...
                                if price > 0 && qty > 0 {
                                    updates.push(L2Update::new(
                                        ...
                                        *symbol,
                                        Side::Bid,
                                        Px::new(price as f64 / 100.0), // Price in paise
                                        Qty::new(qty as f64),
                                        i as u8,
                                    ));
                                }
