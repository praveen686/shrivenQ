...
mod wire;
use wire::{open_reader, open_writer, SyntheticEvent};
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Cmd::Write { path, events, record_size, segment_mb, fsync_ms } => 
            cmd_write(path, events, record_size, segment_mb, fsync_ms),
        Cmd::Replay { path, expect } => 
            cmd_replay(path, expect),
        Cmd::Corrupt { path, flip } => 
            cmd_corrupt(path, flip),
        Cmd::Recover { path } => 
            cmd_recover(path),
        Cmd::Seek { path, probes } => 
            cmd_seek(path, probes),
        Cmd::Verify { path } => 
            cmd_verify(path),
        Cmd::All { path } => 
            cmd_all(path),
...
fn cmd_write(path: PathBuf, events: u64, record_size: usize, segment_mb: usize, fsync_ms: Option<u64>) -> Result<()> {
    fs::create_dir_all(&path).ok();
    let segment_bytes = segment_mb * 1024 * 1024;
    let mut w = open_writer(&path, segment_bytes, fsync_ms).context("open_writer")?;

    let mut rng = StdRng::seed_from_u64(42);
    let mut lat = Histogram::<u64>::new(3)?;
    ...
    for i in 0..events {
        ...
        let ev = SyntheticEvent { ts_ns: now_ns(), payload };
        let a0 = Instant::now();
        w.append_synth(&ev)?;
    ...
    println!("Throughput: {} (target ≥ {} MB/s)", 
        if mb/dt >= pass_mb_s { "✅ PASS" } else { "❌ FAIL" }, pass_mb_s);
    println!("Latency p50: {} (target ≤ {} µs)", 
        if p50 <= pass_p50 { "✅ PASS" } else { "❌ FAIL" }, pass_p50);
    println!("Latency p99: {} (target ≤ {} µs)", 
        if p99 <= pass_p99 { "✅ PASS" } else { "❌ FAIL" }, pass_p99);
    
    Ok(())
}

fn cmd_replay(path: PathBuf, expect: Option<u64>) -> Result<()> {
    let mut r = open_reader(&path).context("open_reader")?;
    ...
    let n = r.replay(|_ts_ns, _len| {
        let us = last.elapsed().as_micros() as u64;
        if us > 0 {  // Skip first event
            let _ = lat.record(us);
        }
        last = Instant::now();
        count += 1;
    ...
    println!("Throughput: {:.2}M events/min", epm / 1e6);
    print_hist("Replay latency", &lat);
    ...
    println!("\n--- Pass/Fail (Dev Laptop Target) ---");
    println!("Throughput: {} (target ≥ {}M events/min)", 
        if epm >= pass_epm { "✅ PASS" } else { "❌ FAIL" }, pass_epm / 1e6);
    println!("Latency p50: {} (target ≤ {} µs)", 
        if p50 <= pass_p50 { "✅ PASS" } else { "❌ FAIL" }, pass_p50);
    println!("Latency p99: {} (target ≤ {} µs)", 
        if p99 <= pass_p99 { "✅ PASS" } else { "❌ FAIL" }, pass_p99);
...
fn cmd_recover(path: PathBuf) -> Result<()> {
    ...
    println!("\n--- Pass/Fail ---");
    println!("Recovery: {} (target ≤ {} ms for {:.2} GB)", 
        if ms <= expected_ms { "✅ PASS" } else { "❌ FAIL" }, expected_ms, gb);
...
fn cmd_seek(path: PathBuf, probes: usize) -> Result<()> {
    ...
    for i in 0..probes {
        ...
        let _ = lat.record(us / 1000); // Convert to ms for display
        
        println!("  Probe {}: seek to {}% took {} ms", i + 1, pct, us / 1000);
    }
    
    print_hist("Seek time (ms)", &lat);
    
    // Pass/Fail check (40ms target)
    let p99 = lat.value_at_percentile(99.0);
    println!("\n--- Pass/Fail ---");
    println!("Seek p99: {} (target ≤ 40 ms)", 
...
fn cmd_verify(path: PathBuf) -> Result<()> {
    ...
    let mut r1 = open_reader(&path)?;
    let mut r2 = open_reader(&path)?;
    ...
    if mismatches > 0 {
        ...
        println!("✅ PASS: All {} events identical across reads", events1.len());
    }
...
fn print_hist(name: &str, h: &Histogram<u64>) {
    ...
    println!("{}: p50={}µs p95={}µs p99={}µs p99.9={}µs max={}µs",
