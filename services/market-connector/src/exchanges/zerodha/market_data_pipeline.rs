                    ...
                    if strike >= min_strike && strike <= max_strike {
                        match option.option_type {
                            Some(OptionType::Call) => {
                                ...
                                // SAFETY: Explicit bounds check ensures strike_rounded fits in i64
                                // SAFETY: i64 bounds comparison safe for f64
                                let strike_i64 = if strike_rounded >= i64::MIN as f64
                                    && strike_rounded <= i64::MAX as f64
                                {
                                    // SAFETY: Bounds checked above
                                    strike_rounded as i64
                                } else {
                                    continue; // Skip invalid strike that would overflow
                                };
