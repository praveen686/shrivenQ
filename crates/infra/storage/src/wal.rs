...
impl Wal {
    ...
    /// # Errors
    ///
    /// Returns an error if directory creation fails or segment initialization fails.
    ...
    /// # Errors
    ///
    /// Returns an error if serialization fails or if the write operation fails.
    ...
    /// # Errors
    ///
    /// Returns an error if segment files cannot be read or iterator creation fails.
    ...
    /// # Errors
    ///
    /// Returns an error if file operations during compaction fail.
...
impl<T: WalEntry> WalIterator<T> {
    ...
    ///
    /// # Errors
    /// Returns an error if segment file cannot be opened or read
...
mod tests {
    ...
    fn test_wal_segment_rotation() -> Result<()> {
        ...
        let mut iter = wal.stream::<WalEvent>(None)?;
        let mut count = 0;
        while iter.read_next_entry()?.is_some() {
            count += 1;
        }
        assert_eq!(count, 100);
