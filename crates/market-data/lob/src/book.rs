...
use crate::price_levels::{DEPTH, SideBook};
...
impl OrderBook {
    ...
    pub fn mid(&self) -> Option<Px> {
        match (self.best_bid(), self.best_ask()) {
            (Some((bid, _)), Some((ask, _))) => {
                let mid_ticks = i64::midpoint(bid.as_i64(), ask.as_i64());
    ...
    pub fn imbalance(&self, depth: usize) -> Option<f64> {
        ...
        if total > 0 {
            // Convert to f64 safely - quantities are in basis points (10000 = 1.0)
            // so max value is well within f64's 53-bit mantissa precision
            let bid_f64 =
                f64::from(i32::try_from(bid_qty.min(i64::from(i32::MAX))).unwrap_or(i32::MAX));
            let ask_f64 =
                f64::from(i32::try_from(ask_qty.min(i64::from(i32::MAX))).unwrap_or(i32::MAX));
            let total_f64 =
                f64::from(i32::try_from(total.min(i64::from(i32::MAX))).unwrap_or(i32::MAX));
...
mod tests {
    ...
    fn test_order_book_basic() {
        ...
        // Add bid levels
        let update = L2Update::new(Ts::from_nanos(1000), Symbol::new(1)).with_level_data(
            Side::Bid,
            Px::new(99.5),
            Qty::new(100.0),
            0,
    ...
    fn test_microprice() {
        ...
        let micro = match book.microprice() {
            ...
            None => {
                assert!(
                    false,
                    "Microprice should be available after setting up book"
                );
                return;
            }
        };
    ...
    fn test_imbalance() {
        ...
        book.apply(
            &L2Update::new(Ts::from_nanos(5000), Symbol::new(1)).with_level_data(
                Side::Ask,
                Px::new(100.0),
                Qty::new(50.0),
                0,
    ...
    fn test_state_hash_deterministic() {
        ...
        let updates = vec![
            L2Update::new(Ts::from_nanos(1), Symbol::new(1)).with_level_data(
                Side::Bid,
                Px::new(99.5),
                Qty::new(100.0),
                0,
            ),
            L2Update::new(Ts::from_nanos(2), Symbol::new(1)).with_level_data(
                Side::Ask,
                Px::new(100.5),
                Qty::new(150.0),
                0,
            ),
            L2Update::new(Ts::from_nanos(3), Symbol::new(1)).with_level_data(
                Side::Bid,
                Px::new(99.4),
                Qty::new(200.0),
                1,
